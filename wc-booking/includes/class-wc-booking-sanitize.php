...
class WC_Booking_Sanitize
{
 ...
 // __construct()
	...
	public function clean()
	{
		 ...
		 */
		do_action('wc_booking_pre_sanitize', $sanitized);
		...
		switch ($this->type)
		{
			...
			case 'price':
				$sanitized = filter_var($this->data, FILTER_SANITIZE_NUMBER_FLOAT);
			...
			case 'url':
				...
				break;
			default:
				error_log('Cannot sanitize unknown type: ' . $this->type);
		} // switch
		 ...
		 */
		do_action('wc_booking_post_sanitize', $sanitized);
 ...
 // clean()
	 ...
	 *      
	 * @param string $date
	 *        	The date as collected from the form field
	 * @param string $format
	 *        	The format to check the date against
 ...
 // validate_date()
	 ...
	 * @since 0.1
	 * @link
	 *       http://jrtashjian.com/2009/03/code-snippet-validate-a-phone-number/
	 * @param string $phone
	 *        	A phone number string
	 * @return string|bool $phone|FALSE Returns the valid phone number, FALSE
	 *         if not
	 */
